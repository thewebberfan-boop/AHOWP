...
另设“问题图谱”作为跨人物的逻辑阅读层：节点只有观察、问题和答案，哲学家只作为参与者附着其上；“提出问题／回应问题／产生问题”只表达论证结构，编辑性的比较与阅读次序另标“阅读跳转”，不冒充逻辑生成或直接历史影响。当前谱系以 826 个节点和 1033 条关系覆盖泰勒斯至罗素，在结构入口层纳入三卷 76 章与现有 82 位人物；其中 19 章已有精读卡片，结构覆盖不等于逐章文本校勘完成。最后六章由功利主义的共同后果与自由修正进入马克思的生产关系批判，再经柏格森的绵延、詹姆斯和杜威的实用探究，抵达弗雷格—罗素的现代逻辑与分析方法；终端保留逻辑、经验、历史、生活体验和公共纠错如何协作的开放问题。观察领域只在节点详情中解释；历史事件或社会变化确实改变问题条件时，可由观察节点进入历史概览并返回原阅读位置。

问题图谱保留全部原子节点与稳定 ID，同时提供“导览／主干／标准／完整／研究”五档全图阅读密度。方法、自然、自我、社会、终极已经各自建立5个总览问题、10条主线、20个论证组和全部相关节点四层阅读；总结节点明确标记为本站学习重构。单击总结卡查看解释，双击或按 Enter 按 5→10→20→全部逐级展开，触屏也可使用展开按钮。多选主题时，各主题成为共享历史阶段序列上的并列泳道，时间标识直接使用历史概览中的阶段名称与年代，跨主题的原始关系与共享节点直接连线。

“自我”首轮一致性修订把167个节点明确归入论证组；粗层连线由实际关系及共享节点生成，不再顺次连卡。人物、流派和历史页的“把这页接回整体”入口共用图谱原文，并支持定位与返回。当前档位标为“总览5／主线10／论证组20／全部节点”。全站结构盘点、抽查依据、剩余内容缺口见 [一致性审查记录](docs/CONTENT_COHERENCE_AUDIT.md)。

中文负责解释，中英双语主要用于人名、地名和哲学概念。网站支持本地运行、离线阅读和 Vercel 部署。

2026-09-03：全部82位哲学家已统一为“核心问题目录＋共用图谱＋同源阅读面板”。155条旧推导卡片退出维护，改为132个精选问题入口；观点正文直接引用既有图谱，保留人物参与说明与论证限制。取舍以降低学习成本为准，不作无损堆积。范围、逐人映射与验证见 [人物核心阅读全量记录](docs/PHILOSOPHER_READING_REFACTOR.md)。
...
- 哲学家页面以核心问题、同源图谱及思想关系组织阅读，概念与必要限定按需展开；不把并列解释硬写成线性推导，也不再单独维护旧推导卡片。
