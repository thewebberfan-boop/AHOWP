...
问题图谱有独立的内容与实现手册 `docs/PROBLEM_MAP_GUIDE.md`。当前谱系由泰勒斯延伸至贝克莱；另一台电脑接手后应先确认内容审计仍报告 1 条谱系、34 个内部维护分组、556 个原子节点和 659 条原始连线，并且第一卷 30 章、现有 29 位第一卷人物、第二卷全部 15 章及 24 位参与人物，以及第三卷第一至十六章和当前十五位参与人物全部覆盖。`app/problem-map-view-data.ts` 另行保存导览／主干／标准／完整／研究五档说明、问题家族锚点、历史背景带、解释边界和并行答案扇面；这些都不能改写原子节点与稳定 ID。继续下一阶段时，从终端节点“物质基质被经验来源审查取消后，精神、因果能力和持续自我能否免于同样审查？”进入第三卷第十七章休谟，并同步判断新节点属于导览锚点、连续链还是分叉接口；新增第二条谱系前，必须先实现 `problemMapId` 选择、会话恢复、搜索定位和返回链，不能只向数据数组追加对象。观察的领域标签只出现在节点详情；新增历史或社会观察时，使用经过审计的历史阶段／回应／事件 ID 建立历史概览入口，并测试返回原节点。
