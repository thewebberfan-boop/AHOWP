...
主题筛选回答“看哪类问题”，阅读层级回答“展开到什么程度”。五个按钮按 `方法 → 自然 → 自我 → 社会 → 终极` 排列，五个主题均已启用。

每个主题各有5个总览问题、10条主线、20个论证组。数量是当前编排，不是思想史规律。总览的卡面给出具体提问和简明回答；详情交代分歧与继续追问。最细一层显示该主题选编的原始观察、问题和答案，不代表穷尽该领域。

| 主题 | 原子节点 | 内部关系 | 内容重心 |
| --- | ---: | ---: | --- |
| 方法 | 224 | 243 | 假说与论证、释经与辩难、模型与实验、经验批判、历史／实践／逻辑分析 |
| 自然 | 200 | 220 | 构成与变化、原因、公共检验、知识边界、生命与行动 |
| 自我 | 167 | 181 | 生活与责任、意志与人格、认识主体、形成与承认、时间与经验 |
| 社会 | 211 | 221 | 共同生活、权威边界、安全与权利、社会自由、制度纠错 |
| 终极 | 173 | 186 | 最高根据、恶与时间、理性与信仰、保证与限度、痛苦与意义 |

五主题合并为648个不同节点；共享节点不是复制出来的相似段落。父卡严格取子卡节点并集，每层覆盖同一批知识。阶段只作历史坐标，不用于整批筛入节点。粗层箭头由跨组原子边聚合，虚线表示共享节点；详情提供依据和核对入口，不按卡片相邻机械连线。

数据位置：
- `problem-map-self-data.ts` 和 `self-reading-data.ts` 保存自我树、节点归属与首轮28条阅读连接；
- `method-reading-data.ts`、`nature-reading-data.ts`、`society-reading-data.ts`、`ultimate-reading-data.ts` 保存四个扩展主题的具体问题、回答、边界、入口与确切节点成员；
- `topic-reading-edges.ts` 累计增加58条有解释的编辑性阅读关系；加上 `self-reading-data.ts` 的28条首轮连接，与原947条论证关系合并后全图为1033条边，未改原始节点ID。这86条编辑边统一使用“阅读跳转”，证据性质只允许“同题并列”或“本站推演”，不再借用“产生问题”语义，也不冒充直接历史影响；
- `reading-topics-data.ts` 提供统一分级、节点并集、主题交集和导航偏好；
- `knowledge-paths.ts` 聚合粗层关系及人物／流派／历史反向入口；`problem-map.tsx` 的 `ProblemGraphCanvas` 同时服务完整图谱与相关节点子图，`problem-graph-controls.tsx` 的 `ProblemGraphControls` 统一主题和阅读层级控制，`knowledge-connections.tsx` 只负责上下文过滤、局部选择状态、节点摘要和返回完整图谱的入口。

多选时每个主题保留自己的总结树，例如三个主题在5层显示15张卡。主题不再首尾纵向拼接，而是在同一条由早到晚的纵向序列上各占一条横向泳道；同一层级的第1—5／10／20组共享行坐标。时间标签由该行实际原子节点所属的问题阶段汇总，再映射到 `historyStages` 的正式阶段名称和年代；不会显示抽象的“共同时段”，也不把阶段当作主题成员筛选器。跨主题原子边持续画为穿越泳道的箭头，共享总结节点画为虚线；同主题关系则随当前卡片突出，避免细层变成无法辨认的线团。双击仍沿当前主题卡片下钻。全部层取节点并集，但仍保留相同的主题列和纵向历史顺序；共有原子节点只出现一次，以当前可见列的稳定负载均衡结果确定显示列，虚线边框和“共享N”标记提示其多重归属。不得绘制穿过未选节点的伪直接关系。

顶部控制区采用两层结构：上层只放“阅读主题”与“阅读层级”（全图模式则为组织尺度与局部聚焦），下层保留节点和箭头图例。不要在控制区末尾追加操作说明长句；说明应进入按钮标题、可访问标签或图面证据栏。

单击选中，空格选中，双击或 Enter 按5→10→20→全部展开；按钮提供同样的触屏操作。进入20层下一级时定位卡片的具体入口。左箭头按全部→20→10→5回到一级；共享节点保留到达时的分支，若在原始图中改选另一组节点，则按新节点重新定位。5层禁用上返。

各主题分别保存 `ahowp-problem-map-{topic}-compression`、`-summary` 与 `-node`；所选主题集合和当前主题单独保存。自我旧版50档迁移为全部。无效或串入其他主题的偏好回到本主题有效入口。跨人物、流派、历史和章节返回时，恢复主题偏好与图内滚动，不覆盖全图密度设置。

“把这页接回整体”在75个人物页、27个流派页、11个历史阶段提供有依据的主题入口。默认显示一个主题，也可像完整图谱一样多选或切到“全部相关”；没有匹配的主题禁用。子图直接复用完整问题图谱的控制条、节点、边、历史背景带、选中态与证据图例，不能另建一套相似控件。5／10／20层只显示底层成员与本页相关的总结节点，全部层显示相关原子节点；单击、双击／Enter展开、层级按钮和左箭头返回的语义与完整图谱一致。单击原子节点在图下查看同源摘要，再以“在完整图谱中定位”进入该节点。流派入口同时核对成员和原书章节，参与者观点不冒充全派共识。原子详情的主题按钮可把同一个节点放回另一个主题的论证组。

内容审计与回归核对五主题每层覆盖、引用、入口、关系语法、网络可达性、共享节点、跨页同源内容、层级返回和独立偏好。可达性包括明确的比较与本站推演，不证明存在一条单一历史影响链。思想史判断仍需阅读核验。详见 [主题内容与边界](TOPIC_READING_AUDIT.md)。
...
内容审计必须验证 ID 唯一、边端点存在、三种论证方向与关系匹配、编辑边明确标为“阅读跳转”、答案具有 `answerRole`、每个观察具有领域元数据且没有论证入边、历史链接指向存在的阶段／回应／事件、答案至少有问题论证入边、问题至少由观察或答案提出。来源目录必须具有有效可点击 URL 和用途说明；已有 `sourceRefs` 必须指向目录中真实来源并带页码、章节或段落定位。三卷全部 76 章和现有 82 位人物还必须全部被谱系覆盖。呈现元数据还必须验证五档密度均有说明、五个问题家族的导览列与锚点有效、每个维护阶段具有现存历史阶段映射、解释边界指向现存节点，以及并行答案扇面的成员确实由共同问题以“回应问题”连接。当前内容基线为 1 条谱系、49 个维护分组、826 个原子节点、1033 条关系。
