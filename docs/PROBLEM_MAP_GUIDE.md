...
当前先完成“自我”试验，其他四个主题在界面中保留顺序但暂不启用。`app/problem-map-self-data.ts` 保存自我相关原子节点、严格嵌套的5／10／20总结树，以及每张20层卡片的显式原子入口 `entryNodeId`；`app/problem-map-self.tsx` 负责总结层的确定性历史主线和原子下钻。四种分辨率为：

1. **5**：五个核心问题，卡面直接显示具体提问和简明回答比较，详情解释分歧与继续追问的理由；
2. **10**：十个主要转折，把每条主线拆成历史换形；
3. **20**：二十个关键接口，保留主要分叉与回应；
4. **全部**：显示全部自我相关原子节点，并只沿自我标签允许的路径折叠连线。
...
内容审计必须验证 ID 唯一、边端点存在、三种方向与关系匹配、答案具有 `answerRole`、每个观察具有领域元数据且没有入边、历史链接指向存在的阶段／回应／事件、答案至少有问题入边、问题至少由观察或答案提出；三卷全部 76 章和现有 82 位人物还必须全部被谱系覆盖。呈现元数据还必须验证五档密度均有说明、五个问题家族的导览列与锚点有效、每个维护阶段具有现存历史阶段映射、解释边界指向现存节点，以及并行答案扇面的成员确实由共同问题以“回应问题”连接。当前内容基线为 1 条谱系、49 个维护分组、826 个原子节点、947 条原始边。
