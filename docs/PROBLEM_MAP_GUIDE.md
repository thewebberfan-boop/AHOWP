...
主题筛选回答“看哪类问题”，阅读层级回答“展开到什么程度”。五个按钮按 `方法 → 自然 → 自我 → 社会 → 终极` 排列。目前自然、自我、社会、终极已启用，方法仅保留接口，未宣称完成独立路径。

每个主题各有5个总览问题、10条主线、20个论证组。数量是当前编排，不是思想史规律。总览的卡面给出具体提问和简明回答；详情交代分歧与继续追问。最细一层显示该主题选编的原始观察、问题和答案，不代表穷尽该领域。

| 主题 | 原子节点 | 内部关系 | 内容重心 |
| --- | ---: | ---: | --- |
| 自然 | 200 | 219 | 构成与变化、原因、公共检验、知识边界、生命与行动 |
| 自我 | 167 | 181 | 生活与责任、意志与人格、认识主体、形成与承认、时间与经验 |
| 社会 | 211 | 220 | 共同生活、权威边界、安全与权利、社会自由、制度纠错 |
| 终极 | 173 | 182 | 最高根据、恶与时间、理性与信仰、保证与限度、痛苦与意义 |

四主题合并为576个不同节点；共享节点不是复制出来的相似段落。父卡严格取子卡节点并集，每层覆盖同一批知识。阶段只作历史坐标，不用于整批筛入节点。粗层箭头由跨组原子边聚合，虚线表示共享节点；详情提供依据和核对入口，不按卡片相邻机械连线。

数据位置：
- `problem-map-self-data.ts` 和 `self-reading-data.ts` 保存自我树、节点归属与首轮28条阅读连接；
- `nature-reading-data.ts`、`society-reading-data.ts`、`ultimate-reading-data.ts` 保存三主题的具体问题、回答、边界、入口与确切节点成员；
- `topic-reading-edges.ts` 增加33条有解释的阅读关系：27条同题并列、6条本站推演。与原关系合并后全图1008条边，未改原始节点ID；
- `reading-topics-data.ts` 提供统一分级、节点并集、主题交集和导航偏好；
- `knowledge-paths.ts` 聚合粗层关系及人物／流派／历史反向入口；`knowledge-connections.tsx` 直接显示共用节点原文。

多选时每个主题保留自己的总结树，例如三个主题在5层显示15张卡。卡面标出所属主题，双击沿该卡下钻；10／20层和多主题总览只画当前卡片的关系。全部层取节点并集，共有节点只出现一次，不绘制穿过未选节点的伪直接关系。
...
“把这页接回整体”在72个人物页、27个流派页、11个历史阶段提供有依据的主题入口。一次只展开一个主题，避免四倍正文堆积；没有匹配的主题不显示。流派入口同时核对成员和原书章节，参与者观点不冒充全派共识。原子详情的主题按钮可把同一个节点放回另一个主题的论证组。

内容审计与回归核对四主题每层覆盖、引用、入口、关系语法、网络可达性、共享节点、跨页同源内容、层级返回和独立偏好。可达性包括明确的比较与本站推演，不证明存在一条单一历史影响链。思想史判断仍需阅读核验。详见 [三主题内容与边界](TOPIC_READING_AUDIT.md)。
...
内容审计必须验证 ID 唯一、边端点存在、三种方向与关系匹配、答案具有 `answerRole`、每个观察具有领域元数据且没有入边、历史链接指向存在的阶段／回应／事件、答案至少有问题入边、问题至少由观察或答案提出；三卷全部 76 章和现有 82 位人物还必须全部被谱系覆盖。呈现元数据还必须验证五档密度均有说明、五个问题家族的导览列与锚点有效、每个维护阶段具有现存历史阶段映射、解释边界指向现存节点，以及并行答案扇面的成员确实由共同问题以“回应问题”连接。当前内容基线为 1 条谱系、49 个维护分组、826 个原子节点、1008 条原始边。
