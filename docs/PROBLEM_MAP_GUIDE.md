...
当前只有一条覆盖整部第一卷、第二卷和第三卷前八章的谱系：

- ID：`difference-change-knowledge`
- 标题：从多样与变化到人工人格与主权
- 范围：第一卷全部 30 章、第二卷全部 15 章与第三卷第一至八章；泰勒斯至霍布斯
- 数据维护分组：27 个
- 节点：452 个，其中观察 53、问题 203、答案 196
- 有向边：544 条
- 人物覆盖：现有第一卷 29 位人物全部至少连接一个节点；第二卷由斐洛至奥卡姆的 23 位既有人物，以及帕多瓦的马西略全部进入图谱；第三卷前八章连接马基雅维利、伊拉斯谟、莫尔、路德、哥白尼、开普勒、伽利略、培根和霍布斯，笛卡尔作为下一章接口进入；威克里夫作为无独立人物页的参与者附着于思想节点
- 章节覆盖：`b1-01` 至 `b1-30`、`b2-01` 至 `b2-15`、`b3-01` 至 `b3-08` 全部至少连接一个节点

二十七个分组只服务数据维护与搜索定位，不再作为页面导航或用户必须接受的哲学分类。页面直接展示完整关系图。内容审计会把第一卷章节与人物、第二卷全部章节和 24 位既有人物，以及第三卷第一至八章和当前十位参与人物的覆盖作为硬约束；增加人物或章节后若没有进入谱系，审计必须报错。
...
页面使用一张确定性 SVG 有向图，不是纵向文字列表，也不是自由漂浮的 D3 力导向图：刷新后空间关系保持稳定，分叉和汇合可以直接看见。图按正文可用宽度完整呈现，不使用横向滚动；整个页面只保留纵向阅读。边的说明文字保存在数据中，不直接覆盖在 SVG 连线上。左侧固定区显示当前节点的摘要、压力、后果、参与者、流派和原书章节；观察节点另显示领域说明，以及存在可靠语境时的历史概览入口。来路与去向只由图中有向边表达，左侧不重复普通邻接清单，也不显示二十七个维护分组。
...
内容审计必须验证 ID 唯一、边端点存在、三种方向与关系匹配、答案具有 `answerRole`、每个观察具有领域元数据且没有入边、历史链接指向存在的阶段／回应／事件、答案至少有问题入边、问题至少由观察或答案提出；第一卷 30 章和现有 29 位第一卷人物、第二卷全部 15 章和 24 位参与人物，以及第三卷第一至八章和当前十位参与人物还必须全部被谱系覆盖。呈现元数据还必须验证五档密度均有说明、五个问题家族的导览列与锚点有效、每个维护阶段具有现存历史阶段映射、解释边界指向现存节点，以及并行答案扇面的成员确实由共同问题以“回应问题”连接。当前内容基线为 1 条谱系、27 个维护分组、452 个原子节点、544 条原始边。
