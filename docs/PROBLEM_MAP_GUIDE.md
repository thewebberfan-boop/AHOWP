...
- `topic-reading-edges.ts` 累计增加58条有解释的编辑性阅读关系；加上 `self-reading-data.ts` 的28条首轮连接，与原947条关系合并后全图为1033条边，未改原始节点ID。方法新增的25条关系只使用“同题并列”或“本站推演”，不冒充直接历史影响；
...
内容审计必须验证 ID 唯一、边端点存在、三种方向与关系匹配、答案具有 `answerRole`、每个观察具有领域元数据且没有入边、历史链接指向存在的阶段／回应／事件、答案至少有问题入边、问题至少由观察或答案提出；三卷全部 76 章和现有 82 位人物还必须全部被谱系覆盖。呈现元数据还必须验证五档密度均有说明、五个问题家族的导览列与锚点有效、每个维护阶段具有现存历史阶段映射、解释边界指向现存节点，以及并行答案扇面的成员确实由共同问题以“回应问题”连接。当前内容基线为 1 条谱系、49 个维护分组、826 个原子节点、1033 条关系。
