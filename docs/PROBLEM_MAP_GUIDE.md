...
- `knowledge-paths.ts` 聚合粗层关系及人物／流派／历史反向入口；`knowledge-connections.tsx` 直接显示共用节点原文。

多选时每个主题保留自己的总结树，例如三个主题在5层显示15张卡。主题不再首尾纵向拼接，而是在同一条由早到晚的纵向序列上各占一条横向泳道；同一层级的第1—5／10／20组共享行坐标。时间标签由该行实际原子节点所属的问题阶段汇总，再映射到 `historyStages` 的正式阶段名称和年代；不会显示抽象的“共同时段”，也不把阶段当作主题成员筛选器。跨主题原子边持续画为穿越泳道的箭头，共享原子节点画为虚线；同主题关系则随当前卡片突出，避免细层变成无法辨认的线团。双击仍沿当前主题卡片下钻。全部层取节点并集，共有节点只出现一次，不绘制穿过未选节点的伪直接关系。
...
“把这页接回整体”在75个人物页、27个流派页、11个历史阶段提供有依据的主题入口。一次只展开一个主题，避免五倍正文堆积；没有匹配的主题不显示。流派入口同时核对成员和原书章节，参与者观点不冒充全派共识。原子详情的主题按钮可把同一个节点放回另一个主题的论证组。
