...
当前先完成“自我”试验，其他四个主题在界面中保留顺序但暂不启用。`app/problem-map-self-data.ts` 保存自我相关原子节点、50个原子地标，以及严格嵌套的5／10／20总结树；`app/problem-map-self.tsx` 负责总结层的确定性历史主线和原子下钻。五种分辨率为：

1. **5**：五个主矛盾，建立从灵魂、意志到发展中自我的最低成本骨架；
2. **10**：十个主要转折，把每条主线拆成历史换形；
3. **20**：二十个关键接口，保留主要分叉与回应；
4. **50**：五十个精选原子地标，恢复具体问题与答案；
5. **全部**：显示全部自我相关原子节点，并只沿自我标签允许的路径折叠连线。

5／10／20层的总结节点必须明确标记“本站学习重构”，并保存覆盖的问题阶段；它们不能加入原子节点数组，也不能冒充历史人物的原话或新的原始关系。点击总结节点可查看它覆盖的历史段落、参与者、章节与原子节点；进入原子节点后恢复现有详情、人物／流派／章节下钻和返回链。
...
内容审计同时验证主题顺序、五档分辨率、50节点地标数量、总结树的5／10／20严格节点数、原子及阶段 ID 存在性，以及5节点层覆盖全部自我相关原子节点。只有完成真实阅读校正后，才能把同一结构复制到方法、自然、社会与终极。
