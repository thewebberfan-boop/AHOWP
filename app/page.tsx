...
export default function Home() {
  ...
  const searchResults = useMemo<SearchResult[]>(() => {
    ...
    ancientDifferenceProblemMap.phases.forEach((phase) => {
      if (includesText([
        ...
        ...phase.nodes.flatMap((node) => [node.kind, node.title, node.summary, node.pressure, node.consequence, node.connection, ...node.participants.flatMap((participant) => [participant.name, participant.role])]),
  ...
  const openProblemPhase = (id: string) => {
    ...
    setMode("problems");
    setPendingProblemTargetId(`problem-${phase.id}`);
    setQuery("");
    setCopied(false);
  };
  ...
  const mobileRailItems: MobileRailItem[] = mode === "history"
    ...
    : mode === "schools"
      ...
      : mode === "philosophers"
        ...
        : mode === "problems"
          ? ancientDifferenceProblemMap.phases.map((phase) => ({
            key: `problem:${phase.id}`,
            badge: String(phase.order).padStart(2, "0"),
            marker: `问题阶段 ${String(phase.order).padStart(2, "0")} · ${phase.label}`,
            title: phase.title,
            detail: phase.question,
            onSelect: () => openProblemPhase(phase.id),
          }))
        : [];
    ...
    <main className={`app-shell app-mode-${mode}${showMobileSearch ? " mobile-search-open" : ""}`}>
      <aside className="sidebar">
        ...
        <div className="sidebar-scroll" ref={sidebarScrollRef}>
          ...
          : showProblemSidebar ? <nav className="problem-map-index" aria-label="问题图谱阶段索引"><div className="results-label"><span>问题谱系 · 当前试验路径</span><b>{ancientDifferenceProblemMap.phases.length}</b></div><div className="structure-notice">节点是问题与思想动作；人物只是参与者</div>{ancientDifferenceProblemMap.phases.map((phase) => <button className={phase.id === problemPhaseId ? "stage-link active" : "stage-link"} data-sidebar-focus={`problem:${phase.id}`} key={phase.id} onClick={() => openProblemPhase(phase.id)}><span className="stage-index">{String(phase.order).padStart(2, "0")}</span><span><small>{phase.label}</small><b>{phase.title}</b><em>{phase.question}</em></span><i>{phase.nodes.length} 节点</i></button>)}</nav>
      ...
      <section className="reading-pane">
        ...
        {mode === "problems" && <ProblemMapView activePhaseId={problemPhaseId} onPhaseChange={observeProblemPhase} onPhilosopher={(id) => openInlineEntity({ kind: "philosopher", id })} onChapter={openChapter} showEnglish={showEnglishTerms} />}
...
function HistoryView({ stage, response, onResponse, onSchool, onPhilosopher, onChapter, showEnglish, onTerm }: { stage: HistoryStage; response: ResponseNode; onResponse: (id: string) => void; onSchool: (id: string) => void; onPhilosopher: (id: string) => void; onChapter: (id: string) => void; showEnglish: boolean; onTerm: (term: TermEntry) => void }) {
  ...
  return <article className="history-page page-wrap">
    ...
    <section className="outputs-section"><div><p className="section-label">06 · 这个阶段留下了什么</p><div className="legacy-list">{stage.legacy.map((item) => <span key={item}><TermText text={item} showEnglish={showEnglish} onTerm={onTerm} /></span>)}</div></div><div><p className="section-label">少数值得跨层保留的连接</p><div className="long-links">{longLinks.slice(linkStart, linkStart + 3).map((link) => <div key={`${link.from}-${link.to}`}><b><TermText text={link.from} showEnglish={showEnglish} onTerm={onTerm} /></b><span>{link.label} · 权重 {link.weight}</span><b><TermText text={link.to} showEnglish={showEnglish} onTerm={onTerm} /></b></div>)}</div></div></section>
...
function TermText({ text, showEnglish, onTerm, interactive = true }: { text: string; showEnglish: boolean; onTerm: (term: TermEntry) => void; interactive?: boolean }) {
  ...
  return <>{parts.map((part, index) => {
    ...
    if (!interactive) {
      return <span className="term-static" key={`${term.id}-${index}`}><span>{term.zh}</span>{showEnglish && label.english && <small>{label.english}</small>}</span>;
    }
    if (term.entity && onEntity) {
      ...
      return <span className="term-token entity-token" key={`${term.id}-${index}`} role="button" tabIndex={0} onClick={openEntity} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openEntity(event); }} aria-label={`打开${entityLabel}：${term.zh}，${label.english}`}><span>{term.zh}</span>{showEnglish && label.english && <small>{label.english}</small>}<i aria-hidden="true">↗</i></span>;
    }
